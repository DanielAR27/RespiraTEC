...
export default function AdminEventosNew() {
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      if (response.success) {
        navigate('/admin/eventos', { 
          state: { 
            successMessage: `Se ha creado el evento "${response.data.titulo}" exitosamente` 
          } 
        });
      }
    } catch (error) {
      setErrorToast(error.message || 'Ocurrió un error al crear el evento');
    } finally {
  ...
  return (
    <main className="p-4 md:p-8 max-w-4xl mx-auto space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-[#243e7b]">Nuevo Evento</h1>
          <p className="text-gray-500 text-sm mt-1">Completa los datos para registrar un nuevo evento en el sistema.</p>
        </div>
      </div>
