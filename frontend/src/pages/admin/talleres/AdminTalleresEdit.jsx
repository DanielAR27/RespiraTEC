...
import { getTaller, updateTaller } from '../../../api/talleres';

export default function AdminTalleresEdit() {
  const { id } = useParams();
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      if (response.success) {
        navigate('/admin/talleres', { 
          state: { 
            successMessage: `Se ha actualizado el taller "${response.data.titulo}" exitosamente` 
          } 
        });
      }
    } catch (error) {
      setErrorToast(error.message || 'Ocurrió un error al actualizar el taller');
    } finally {
  ...
  return (
    <main className="p-4 md:p-8 max-w-4xl mx-auto space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-[#243e7b]">Editar Taller</h1>
          <p className="text-gray-500 text-sm mt-1">Modifica los detalles del taller existente.</p>
        </div>
      </div>
